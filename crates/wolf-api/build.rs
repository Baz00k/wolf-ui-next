...
fn main() {
    let manifest_dir = PathBuf::from(env::var_os("CARGO_MANIFEST_DIR").expect("CARGO_MANIFEST_DIR set"));
    ...
    let spec: serde_json::Value = serde_json::from_str(&schema).expect("parse Wolf OpenAPI schema JSON");
    ...
    let generated = generated.replace("#[derive(Debug, Clone, Deserialize, Serialize)]", "#[derive(Debug, Clone, PartialEq, Deserialize, Serialize)]");
